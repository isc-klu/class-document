...
export class Reader {
    ...
    public read(n: number = 1): Result<string> {
...
export function read(n: number = 1) {
    return (reader: Reader): Result<String> => reader.read(n);
}
...
export function seqN<T>(...ps: Parser<T>[]): Parser<T[]> {
    return ps.reduce((acc, p) => map(seq2(p, acc), cons), succ([] as T[]));
}

export function altN<T>(...ps: Parser<T>[]): Parser<T> {
    return ps.reduce((acc, p) => alt2(p, acc), fail);
}

export function firstN(n: number, p: (x: string) => boolean = (_) => true): Parser<string> {
    function* g(reader: Reader) {
        const prefix = reader.read(n);
        if (p(prefix.value)) {
            yield prefix
        }
        return null;
...
export const cons = <T>([x, xs]: [T, T[]]) => [x].concat(xs)

export function zeroOrMore<T>(x: Parser<T>): Parser<T[]> {
...
export const oneOrMore = <T>(p: Parser<T>) => {
    return map(seq2(p, zeroOrMore(p)), cons)
}

export function sepList<I,S>(pi: Parser<I>, ps: Parser<S>): Parser<[I[],S[]]> {
    return alt2(
        map(seq2(pi, zeroOrMore(seq2(ps, pi))), ([x, xys]) => {
            const xs: I[] = [x];
            const ys: S[] = [];
...
export const char = firstN(1, (_) => true);
export const butNL = firstN(1, (x) => /[^\n]/.test(x));
export const space = firstN(1, (x) => /\s/.test(x));
export const alpha = firstN(1, (x) => /[a-z]/.test(x));
export const ALPHA = firstN(1, (x) => /[A-Z]/.test(x));
export const AlPhA = firstN(1, (x) => /[a-z]/i.test(x));
export const numeral = firstN(1, (x) => /[0-9]/.test(x));
export const alphanumeral = alt2(alpha, numeral);
